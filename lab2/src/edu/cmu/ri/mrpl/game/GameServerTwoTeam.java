...
public class GameServerTwoTeam {
	protected static MazeWorld mw;
	protected static GameDisplayTwoTeam disp;
	protected static boolean flipped;
	protected static RealPose2D maze_origin, maze_origin_flipped;
	...
	if (lastMessage.contains("POSE"))
		...
		if (tokens[1].equals(myFriends[0]))
		    ...
		    robotNumber = 1;
		else if (tokens[1].equals(myFriends[2]))
		    robotNumber = 2;
		else if (tokens[1].equals(myFriends[3]))
		    robotNumber = 3;
		...
		if(robotNumber > 1)
			p = RealPose2D.multiply(maze_origin_flipped, p);
		else
		...
		int mazeX = Integer.parseInt(tokens[2]);
		int mazeY = Integer.parseInt(tokens[3]);
		MazeWorld.Direction mazeDir = MazeWorld.Direction.valueOf(tokens[4]);
	...
	if(args.length != 4 && args.length != 6 && args.length != 2 )
	    {
		System.out.println("Usage: java GameServerTwoTeam mazeFile MatchName RobotName1 RobotName2 RobotName3 RobotName4 (logfile)");
		System.out.println("This will run the server, and log to logFile if provided\n");
		System.out.println("java GameServerTwoTeam mazeFile logFile");
	...
	maze_origin_flipped = new RealPose2D(mw.getWidth()-1, mw.getHeight()-1, Math.PI);
	...
	if (args.length == 2)
		...
		String matchName = "Log Playback";
		System.out.println("Parsed Robots Names From File:  " + myFriends[0] + "  and  " + myFriends[1]);

		disp = new GameDisplayTwoTeam(mw, matchName, myFriends);
		BufferedReader input = null;
		try {
	...
	if (args.length >= 4)
		...
		String myName = "GameServerForTeam" + matchName;
		String [] myFriends = {"GC4" + args[2],"GC4" + args[3],"GC4"+args[4],"GC4"+args[5]};
		String [] myFriends_disp = {args[2],args[3],args[4],args[5]};
    ...
    disp = new GameDisplayTwoTeam(mw, matchName, myFriends_disp);
		...
		while(true)
			...
			if (lastMessage != null)
				...
				parseMessageAndUpdateGraphics(lastMessage, myFriends);
			    }
		    }
